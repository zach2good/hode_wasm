...
struct Game
{
    ...
    // 32*24 pitch=32
    static uint32_t screenGridOffset(int x, int y) { return ((y & ~7) << 2) + (x >> 3); }
    ...
    int updateSwitchesLar_toggle(
        bool flag, uint8_t dataNum, int screenNum, int switchNum, int anim, const BoundingBox* switchBoundingBox);
    void dumpSwitchesLar(
        int switchesCount,
