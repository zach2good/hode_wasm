...
static const uint8_t andyOp1LookupTable[] = {
    0,  0,  0,  0,  1,  1,  2,  2,  3,  4,  3,  4,  2,  2,  1,  1,  4,  3,  4,  3,  5,  6,  7,  8,  7,  8,  5,  6,
    8,  7,  6,  5,  6,  5,  8,  7,  9,  9,  9,  9,  10, 10, 11, 11, 12, 13, 12, 13, 11, 11, 10, 10, 13, 12, 13, 12,
    14, 15, 16, 17, 15, 14, 17, 16, 16, 17, 14, 15, 17, 16, 15, 14, 18, 19, 18, 19, 19, 18, 19, 18, 20, 20, 21, 21,
    21, 21, 20, 20, 22, 23, 22, 23, 23, 22, 23, 22, 24, 24, 25, 25, 25, 25, 24, 24, 26, 26, 26, 26, 27, 27, 27, 27
};
...
static const int _andyMoveTable35[] = {
    0,   0,  8, 0,  -8, 0,   8,   8,   -8, 8, 0,   8, 8,  -8, -8,  -8, 0, -8, 16, 0,   -16, 0,   16, 16,
    -16, 16, 0, 16, 16, -16, -16, -16, 24, 0, -24, 0, 24, 24, -24, 24, 0, 24, 24, -24, -24, -24, 0,  0,
};

static const int _andyMoveTable37[] = {
    -1021, 1027, 2048, -1027, 1021, 2048, -1021, -2048, -3069, -1027, -2048, -3075
};
...
int Game::moveAndyObjectOp3(int op)
{
    ...
    case 31:
    {
        ...
        if (((_andyMoveState[24 + index1] & _andyMoveState[8 + index1]) & 8) == 0
            || (_andyMoveState[8 + index2] & 1) != 0)
        {
            return 0;
        }
    }
    ...
    case 38:
        ...
        if ((_screenMaskBuffer[_andyMaskBufferPos3 + 1533] | _screenMaskBuffer[_andyMaskBufferPos3 + 1534]
             | _screenMaskBuffer[_andyMaskBufferPos3 + 1535])
            & 8)
        {
            if ((_screenMaskBuffer[_andyMaskBufferPos3 + 2562] | _screenMaskBuffer[_andyMaskBufferPos3 + 2561]
                 | _screenMaskBuffer[_andyMaskBufferPos3 + 2560])
                & 8)
            {
                return 1;
            }
        ...
        if ((_screenMaskBuffer[_andyMaskBufferPos3 + 1539] | _screenMaskBuffer[_andyMaskBufferPos3 + 1538]
             | _screenMaskBuffer[_andyMaskBufferPos3 + 1537])
            & 8)
        {
            if ((_screenMaskBuffer[_andyMaskBufferPos3 + 2558] | _screenMaskBuffer[_andyMaskBufferPos3 + 2559]
                 | _screenMaskBuffer[_andyMaskBufferPos3 + 2560])
                & 8)
            {
                return 1;
    ...
    case 50:
        for (int i = 0; i < 3; ++i)
        {
            if (((_screenMaskBuffer[_andyMoveTable8[i] + _andyMaskBufferPos4]
                  | _screenMaskBuffer[_andyMoveTable8[i] + _andyMaskBufferPos5])
                 & 1)
                != 0)
            {
                return 1;
    ...
    case 51:
        for (int i = 0; i < 3; ++i)
        {
            if (((_screenMaskBuffer[_andyMoveTable8[i] + _andyMaskBufferPos5]
                  | _screenMaskBuffer[_andyMoveTable8[i] + _andyMaskBufferPos4])
                 & 1)
                != 0)
            {
                return 0;
    ...
    case 67:
    {
        ...
        int mask = _screenMaskBuffer[_andyMaskBufferPos0] | _screenMaskBuffer[_andyMaskBufferPos1]
            | _screenMaskBuffer[_andyMaskBufferPos2];
        if (mask & 6)
        {
            return 1;
        }
        mask = _screenMaskBuffer[_andyMaskBufferPos0 + offset] | _screenMaskBuffer[_andyMaskBufferPos1 + offset]
            | _screenMaskBuffer[_andyMaskBufferPos2 + offset];
        if (mask & 6)
        {
            return 1;
        }
