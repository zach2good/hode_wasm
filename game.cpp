...
static BoundingBox _screenTransformRects[] = { { 0, 0, 0, 0 },     { 0, 0, 255, 128 }, { 0, 10, 154, 126 },
                                               { 0, 0, 107, 36 },  { 0, 1, 78, 29 },   { 14, 7, 249, 72 },
                                               { 14, 0, 255, 72 }, { 0, 0, 255, 144 }, { 0, 0, 255, 144 },
                                               { 0, 69, 255, 191 } };
...
void Game::decodeShadowScreenMask(LvlBackgroundData* lvl)
{
    ...
    for (int i = lvl->currentShadowId; i < lvl->shadowCount; ++i)
    {
        ...
        if (src)
        {
            ...
            _video->buildShadowColorLookupTable(
                _shadowScreenMasksTable[i].shadowPalettePtr, _video->_shadowColorLookupTable);
            dst += decodedSize;
        }
    }
}
...
void Game::setupBackgroundBitmap()
{
    ...
    lvl->backgroundPaletteId = READ_LE_UINT16(pal);
    pal += 2;
    const uint8_t* pic = lvl->backgroundBitmapTable[num];
    lvl->backgroundBitmapId = READ_LE_UINT16(pic);
    pic += 2;
...
void Game::setupPlasmaCannonPointsHelper()
{
    if (_plasmaCannonPointsSetupCounter == 0)
    {
        ...
        if (_andyObject->anim == 84)
        {
            ...
            {
                const int index = i / 4;
                _plasmaCannonXPointsTable2[2 + index] =
                    (_plasmaCannonPosX[4 + index] - _plasmaCannonXPointsTable1[4 + index]) / 2;
                _plasmaCannonYPointsTable2[2 + index] =
                    (_plasmaCannonPosY[4 + index] - _plasmaCannonYPointsTable1[4 + index]) / 8;
            }
        }
        else
        {
            for (int i = 0; i <= 496; i += 8)
            {
                ...
                _plasmaCannonXPointsTable2[2 + index] =
                    (_plasmaCannonPosX[4 + index] - _plasmaCannonXPointsTable1[4 + index]) / 2;
                _plasmaCannonYPointsTable2[2 + index] =
                    (_plasmaCannonPosY[4 + index] - _plasmaCannonYPointsTable1[4 + index]) / 2;
            }
        }
...
void Game::setupPlasmaCannonPoints(LvlObject* ptr)
{
    ...
    {
        if ((ptr->actionKeyMask & 4) == 0)
        { // not using plasma cannon
...
void Game::updateScreenHelper(int num)
{
    ...
    for (LvlObject* ptr = _screenLvlObjectsList[num]; ptr; ptr = ptr->nextPtr)
    {
        ...
        case 0:
        {
            ...
            if (_res->_isPsx)
            {
                p->framesCount = READ_LE_UINT32(data);
                data += 4;
                ptr->currentSound = READ_LE_UINT32(data);
                data += 4;
                p->nextSpriteData = READ_LE_UINT16(data + 6) + data + 6;
            }
            else
            {
                p->framesCount = READ_LE_UINT16(data);
                data += 2;
                ptr->currentSound = READ_LE_UINT16(data);
                data += 2;
        ...
        case 2:
            ...
            if (_currentLevel == kLvl_rock)
            {
                ...
                default:
                    warning(
                        "updateScreenHelper unimplemented for level %d, state %d",
                        _currentLevel,
                        ptr->objectUpdateType);
                    break;
                }
            }
            else
...
void Game::setAndyLvlObjectPlasmaCannonKeyMask()
{
    ...
    {
        if (_actionDirectionKeyMaskIndex == 0xA4 && !_fadePalette)
        { // game over
...
int Game::clipLvlObjectsBoundingBox(LvlObject* o, LvlObject* ptr, int type)
{
    ...
    case 49:
        ...
        {
            updateBoundingBoxClippingOffset(
                &obj1,
                &obj2,
                _res->getLvlSpriteCoordPtr(ptr->levelData0x2988, ptr->currentSprite),
                (ptr->flags1 >> 4) & 3);
        }
        break;
    case 16:
    ...
    case 48:
        ...
        if (clipBoundingBox(&obj1, &obj2))
        {
            return updateBoundingBoxClippingOffset(
                &obj1,
                &obj2,
                _res->getLvlSpriteCoordPtr(ptr->levelData0x2988, ptr->currentSprite),
                (ptr->flags1 >> 4) & 3);
    ...
    case 19:
        ...
        {
            return updateBoundingBoxClippingOffset(
                &obj2, &obj1, _res->getLvlSpriteCoordPtr(o->levelData0x2988, o->currentSprite), (o->flags1 >> 4) & 3);
        }
        break;
    case 3:
        obj1.x2 = obj1.x1 + o->width - 1;
...
void Game::drawScreen()
{
    ...
    for (int i = 1; i < 8; ++i)
    {
        for (Sprite* spr = _typeSpritesList[i]; spr; spr = spr->nextPtr)
        {
            ...
            {
                _video->decodeSPR(
                    spr->bitmapBits, _video->_shadowLayer, spr->xPos, spr->yPos, (spr->num >> 0xE) & 3, spr->w, spr->h);
            }
        }
    }
    for (int i = 1; i < 4; ++i)
    {
        for (Sprite* spr = _typeSpritesList[i]; spr; spr = spr->nextPtr)
        {
            ...
            {
                _video->decodeSPR(
                    spr->bitmapBits, _video->_frontLayer, spr->xPos, spr->yPos, (spr->num >> 0xE) & 3, spr->w, spr->h);
            }
        }
    }
...
#ifdef EMSCRIPTEN
        emscripten_set_main_loop_arg(gameStep, this, 12, 1);
#else
        gameStep(this);
#endif
        if (g_system->inp.quit)
...
LvlObject* Game::updateAnimatedLvlObjectType0(LvlObject* ptr)
{
    ...
    case 4:
        ++vg->currentFrame;
        if (vg->currentFrame < vg->framesCount)
        { // bugfix: original uses '<=' (out of bounds)
            vg->currentSpriteData = nextSpriteData;
        }
        else
        {
...
LvlObject* Game::updateAnimatedLvlObjectType2(LvlObject* ptr)
{
    ...
    if ((ptr->flags1 & 6) == 2)
    {
        ...
        ptr->yPos +=
            calcScreenMaskDy(ptr->xPos + ptr->posTable[index].x, ptr->yPos + ptr->posTable[index].y, ptr->screenNum);
    }
    if (!ptr->bitmapBits)
    {
        return o;
    ...
    if (_plasmaCannonDirection != 0 && _plasmaCannonLastIndex1 != 0)
    {
        ...
        if (_plasmaCannonExplodeFlag)
        {
            ...
            if ((_rnd._rndSeed & 1) != 0 && addLvlObjectToList3(1))
            {
                ...
                setLvlObjectPosRelativeToPoint(
                    _lvlObjectsList3,
                    0,
                    _plasmaCannonXPointsTable1[_plasmaCannonLastIndex1],
                    _plasmaCannonYPointsTable1[_plasmaCannonLastIndex1]);
            }
        }
        setLvlObjectPosRelativeToPoint(
            ptr,
...
void Game::levelMainLoop()
{
    ...
    if (_cheats != 0)
    {
        ...
        snprintf(
            ...
            "P%d S%02d %d R%d",
            _currentLevel,
            _andyObject->screenNum,
            _res->_screensState[_andyObject->screenNum].s0,
            _level->_checkpoint);
        _video->drawString(
            buffer, (Video::W - strlen(buffer) * 8) / 2, 8, _video->findWhiteColor(), _video->_frontLayer);
    }
    ...
    if (g_system->inp.keyPressed(SYS_INP_ESC) || g_system->inp.exit)
    { // display exit confirmation screen
...
void* Game::getLvlObjectDataPtr(LvlObject* o, int type) const
{
    ...
    case kObjectDataTypeAnimBackgroundData:
        assert(
            o->dataPtr >= &_animBackgroundDataTable[0] && o->dataPtr < &_animBackgroundDataTable[kMaxBackgroundAnims]);
        break;
    case kObjectDataTypeShoot:
        assert(
            o->dataPtr >= &_shootLvlObjectDataTable[0]
            && o->dataPtr < &_shootLvlObjectDataTable[kMaxShootLvlObjectData]);
        break;
...
void Game::setupSpecialPowers(LvlObject* ptr)
{
    ...
    { // release
        ...
        if (va->type == 4)
        {
            ...
            // dx, dy
            static const uint8_t _byte_43E670[16] = { 0x0F, 0x00, 0xF1, 0xF6, 0xF1, 0x0A, 0x0F, 0xF6,
                                                      0x0F, 0x0A, 0xF1, 0x00, 0x00, 0xF1, 0x00, 0x0F };
...
int Game::lvlObjectType0Callback(LvlObject* ptr)
{
    ...
    if (!_hideAndyObjectFlag)
    {
        ...
        {
            ptr->yPos +=
                calcScreenMaskDy(ptr->posTable[7].x + ptr->xPos, ptr->posTable[7].y + ptr->yPos, ptr->screenNum);
        }
    }
...
int Game::lvlObjectType1Callback(LvlObject* ptr)
{
    if (ptr)
    {
        ...
        setLvlObjectPosRelativeToPoint(
            ptr, 0, _plasmaCannonPosX[_plasmaCannonFirstIndex], _plasmaCannonPosY[_plasmaCannonFirstIndex]);
        updateAndyObject(ptr);
        ptr->flags2 = merge_bits(ptr->flags2, _andyObject->flags2, 0x18);
        ptr->flags2 = merge_bits(ptr->flags2, _andyObject->flags2 + 1, 7);
    }
...
int Game::lvlObjectType7Callback(LvlObject* ptr)
{
    ...
    if ((ptr->flags0 & 0x1F) == 1)
    {
        ...
        ptr->xPos += dat->dxPos;
        ptr->yPos += dat->dyPos;
        if (!_hideAndyObjectFlag && ptr->screenNum == _andyObject->screenNum && (_andyObject->flags0 & 0x1F) != 0xB
            && clipLvlObjectsBoundingBox(_andyObject, ptr, 68) && (_mstFlags & 0x80000000) == 0
            && (_cheats & kCheatSpectreFireballNoHit) == 0)
        {
            dat->unk3 = 0x80;
        ...
        if (dat->unk3 != 0x80 && dat->counter != 0)
        {
            if (dat->type == 7)
            {
                ptr->yPos += calcScreenMaskDy(
                    ptr->xPos + ptr->posTable[7].x, ptr->yPos + ptr->posTable[7].y + 4, ptr->screenNum);
            }
        ...
        assert(dat->state < 8);
        static const uint8_t animData1[16] = { 0x06, 0x0B, 0x05, 0x09, 0x05, 0x09, 0x05, 0x09,
                                               0x05, 0x09, 0x06, 0x0B, 0x04, 0x07, 0x04, 0x07 };
        static const uint8_t animData2[16] = { 0x06, 0x0E, 0x05, 0x0E, 0x05, 0x0E, 0x05, 0x0E,
                                               0x05, 0x0E, 0x06, 0x0E, 0x04, 0x0E, 0x04, 0x0E };
...
int Game::lvlObjectType8Callback(LvlObject* ptr)
{
    ...
    else
    {
        ...
        setLvlObjectPosInScreenGrid(ptr, 7);
        if (ptr->screenNum == _currentScreen || ptr->screenNum == _currentLeftScreen
            || ptr->screenNum == _currentRightScreen || o || (_currentLevel == kLvl_lar2 && ptr->spriteNum == 27)
            || (_currentLevel == kLvl_isld && ptr->spriteNum == 26))
        {
            if (ptr->currentSound != 0xFFFF)
            {
...
uint8_t Game::lvlObjectCallbackCollideScreen(LvlObject* o)
{
    ...
    ShootLvlObjectData* dat = (ShootLvlObjectData*)getLvlObjectDataPtr(o, kObjectDataTypeShoot);
    static const uint8_t data1[] = { 0xFF, 0x00, 0x07, 0x00, 0x0F, 0x00, 0x17, 0x00, 0x08, 0x08, 0x00, 0x00, 0xF8,
                                     0xF8, 0xF0, 0xF0, 0x08, 0xF8, 0x00, 0xFF, 0xF8, 0x07, 0xF0, 0x0F, 0xFF, 0x08,
                                     0x07, 0x00, 0x0F, 0xF8, 0x17, 0xF0, 0xFF, 0xF8, 0x07, 0xFF, 0x0F, 0x07, 0x17,
                                     0x0F, 0x08, 0x00, 0x00, 0x00, 0xF8, 0x00, 0xF0, 0x00, 0x00, 0x08, 0x00, 0x00,
                                     0x00, 0xF8, 0x00, 0xF0, 0x00, 0xF8, 0x00, 0xFF, 0x00, 0x07, 0x00, 0x0F };
    static const uint8_t data2[] = { 0x00, 0x00, 0x08, 0x00, 0x00, 0x08, 0x08, 0x08, 0x00, 0x00, 0xF8,
                                     0x00, 0x00, 0x08, 0xF8, 0x08, 0x00, 0x00, 0x08, 0x00, 0x00, 0xF8,
                                     0x08, 0xF8, 0x00, 0x00, 0xF8, 0x00, 0x00, 0xF8, 0xF8, 0xF8 };
    static const int offsets1[] = { 0, 1,   1,   1,   0, -513, -513, -513, 0, 511,  511,  511,  0, -511, -511, -511,
                                    0, 513, 513, 513, 0, -1,   -1,   -1,   0, -512, -512, -512, 0, 512,  512,  512 };
    static const int offsets2[] = { 0, 1, 512, -1, 0, -1, 512, 1, 0, 1, -512, -1, 0, -1, -512, 1 };
...
int Game::lvlObjectSpecialPowersCallback(LvlObject* o)
{
    ...
    if (fl == 1)
    {
        ...
        static const uint8_t animData1[] = { 0x04, 0x14, 0x03, 0x16, 0x03, 0x16, 0x03, 0x16,
                                             0x03, 0x16, 0x04, 0x14, 0x02, 0x18, 0x02, 0x18 };
        static const uint8_t animData2[] = { 0x04, 0x08, 0x03, 0x08, 0x03, 0x08, 0x03, 0x08,
                                             0x03, 0x08, 0x04, 0x08, 0x02, 0x08, 0x02, 0x08 };
...
void Game::initLvlObjects()
{
    ...
    for (int i = 0; i < _res->_lvlHdr.staticLvlObjectsCount; ++i)
    {
        ...
        case 1:
            ...
            {
                warning(
                    "Trying to free _resLvlScreenBackgroundDataTable.backgroundSoundTable (i=%d index=%d)", i, index);
            }
            ptr->xPos = 0;
            ptr->yPos = 0;
            break;
    ...
    for (int i = _res->_lvlHdr.staticLvlObjectsCount;
         i < _res->_lvlHdr.staticLvlObjectsCount + _res->_lvlHdr.otherLvlObjectsCount;
         ++i)
    {
        LvlObject* ptr = &_res->_resLvlScreenObjectDataTable[i];
...
void Game::updateGatesLar(LvlObject* o, uint8_t* p, int num)
{
        ...
        else
        {
            ...
            if (_cl != _dl)
            {
                ...
                else
                {
                    ...
                    _mstLevelGatesMask |= mask;
                    if (o->screenNum != _currentScreen && o->screenNum != _currentLeftScreen
                        && o->screenNum != _currentRightScreen)
                    {
                        o->actionKeyMask = 1;
                    }
                    else
    ...
    uint32_t offset =
        screenMaskOffset(_res->_screensBasePos[o->screenNum].u + x1, _res->_screensBasePos[o->screenNum].v + y1);
    if (h1 < 0)
    {
        h1 = -h1;
    ...
    if (o->screenNum == _currentScreen
        || (o->screenNum == _currentRightScreen && _res->_resLevelData0x2B88SizeTable[_currentRightScreen] != 0)
        || (o->screenNum == _currentLeftScreen && _res->_resLevelData0x2B88SizeTable[_currentLeftScreen] != 0))
    {
    ...
    if ((_cheats & kCheatGateNoCrush) == 0 && o->screenNum == _res->_currentScreenResourceNum
        && o->directionKeyMask == 4)
    {
...
void Game::updateSwitchesLar(int count, uint8_t* switchesData, BoundingBox* switchesBoundingBox, uint8_t* gatesData)
{
    ...
    for (int i = 0; i < count; ++i)
    {
        ...
        {
            if ((switchesData[i * 4 + 1] & 0x10) == 0x10)
            { // can be actioned by a spectre
                updateSwitchesLar_checkSpectre(i, &switchesData[i * 4], &switchesBoundingBox[i], gatesData);
            }
            AndyLvlObjectData* data = (AndyLvlObjectData*)getLvlObjectDataPtr(_andyObject, kObjectDataTypeAndy);
            updateSwitchesLar_checkAndy(
                i, &switchesData[i * 4], &data->boundingBox, &switchesBoundingBox[i], gatesData);
        }
    }
    for (int i = 0; i < count; ++i)
    {
...
int Game::updateSwitchesLar_toggle(
    bool flag, uint8_t dataNum, int screenNum, int switchNum, int anim, const BoundingBox* box)
{
...
void Game::dumpSwitchesLar(
    int switchesCount,
    const uint8_t* switchesData,
    const BoundingBox* switchesBoundingBox,
    int gatesCount,
    const uint8_t* gatesData)
{
    ...
    for (int i = 0; i < switchesCount; ++i)
    {
        ...
        fprintf(
            stdout,
            "switch %2d: screen %2d (%3d,%3d,%3d,%3d) flags 0x%02x sprite %2d gate %2d\n",
            i,
            p[0],
            b->x1,
            b->y1,
...
void Game::updateScreenMaskLar(uint8_t* p, uint8_t flag)
{
    if (p[1] != flag)
    {
        ...
        uint32_t maskOffset =
            screenMaskOffset(_res->_screensBasePos[screenNum].u + p[2], _res->_screensBasePos[screenNum].v + p[3]);
