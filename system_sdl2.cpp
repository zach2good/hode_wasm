...
static const struct
{
    const char* name;
    const Scaler* scaler;
} _scalers[] = { { "nearest", &scaler_nearest }, { "xbr", &scaler_xbr }, { 0, 0 } };
...
struct System_SDL2 : System
{
    ...
    virtual void copyYuv(
        int w, int h, const uint8_t* y, int ypitch, const uint8_t* u, int upitch, const uint8_t* v, int vpitch);
    ...
    if (SDL_LockTexture(_widescreenTexture, 0, &ptr, &pitch) == 0)
    {
        ...
        if (src && tmp)
        {
            for (int i = 0; i < w * h; ++i)
            {
                ...
                src[i] = SDL_MapRGB(
                    _fmt, _gammaLut[pal[color * 3]], _gammaLut[pal[color * 3 + 1]], _gammaLut[pal[color * 3 + 2]]);
            }
...
void System_SDL2::prepareScaledGfx(const char* caption, bool fullscreen, bool widescreen, bool yuv)
{
    ...
    {
        _backgroundTexture =
            SDL_CreateTexture(_renderer, SDL_PIXELFORMAT_YV12, SDL_TEXTUREACCESS_STREAMING, _screenW, _screenH);
