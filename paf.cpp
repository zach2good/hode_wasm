...
static const char* _filenames[] = { "hod.paf", "hod_demo.paf", "hod_demo2.paf", 0 };
...
void PafPlayer::decodeVideoFrameOp0(const uint8_t* base, const uint8_t* src, uint8_t code)
{
    ...
    if (count != 0)
    {
        ...
        do
        {
            ...
            uint32_t end = READ_LE_UINT16(src + 2);
            src += 4;
            end += offset;
            do
            {
    ...
    do
    {
        const uint8_t* src2 = getVideoPageOffset(src[0], src[1]);
        src += 2;
        ...
        for (int x = 0; x < kVideoWidth; x += 4, dst += 4)
        {
            ...
            while (*opcodes)
            {
                ...
                case 5:
                    offset = 0;
                case 6:
                    src2 = getVideoPageOffset(src[0], src[1]);
                    src += 2;
                case 7:
                    mask = *src++;
...
void PafPlayer::decodeAudioFrame(const uint8_t* src, uint32_t offset, uint32_t size)
{
    ...
    {
        warning(
            "Unexpected offset 0x%x wr 0x%x rd 0x%x num %d",
            offset,
            _audioBufferOffsetWr,
            _audioBufferOffsetRd,
            _videoNum);
        assert(offset == 0);
    ...
    if (count != 0)
    {
        ...
        if (sq)
        {
            ...
            if (sq->buffer)
            {
                for (int i = 0; i < count; ++i)
                {
                    decodeAudioFrame2205(
                        src + _audioBufferOffsetRd + i * kAudioStrideSize,
                        kAudioSamples,
                        sq->buffer + i * kAudioSamples * 2);
                }
            }
            sq->next = 0;
...
void PafPlayer::mainLoop()
{
    ...
    for (int i = 0; i < (int)_pafHdr.framesCount; ++i)
    {
        ...
        uint32_t blocksCountForFrame =
            (i == 0) ? _pafHdr.preloadFrameBlocksCount : _pafHdr.frameBlocksCountTable[i - 1];
