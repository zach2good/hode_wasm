...
{
    return config->players[num].levelNum == 0 && config->players[num].checkpointNum == 0
        && config->players[num].cutscenesMask == 0;
}
...
void Menu::loadData()
{
    ...
    if (version == 10)
    {
        ...
        const int size = READ_LE_UINT32(ptr + ptrOffset);
        ptrOffset += 4;
        assert((size % (16 * 10)) == 0);
    ...
    _optionsButtonSpritesCount = READ_LE_UINT32(ptr + ptrOffset);
    ptrOffset += 4;
...
void Menu::drawSprite(const DatSpritesGroup* spriteGroup, const uint8_t* ptr, uint32_t num)
{
    ...
    for (uint32_t i = 0; i < spriteGroup->count; ++i)
    {
        ...
        if (num == i)
        {
            ...
            {
                _video->decodeSPR(
                    ptr + 8, _video->_frontLayer, ptr[0], ptr[1], 0, READ_LE_UINT16(ptr + 4), READ_LE_UINT16(ptr + 6));
            }
            break;
        }
        ptr += size + 2;
...
void Menu::drawSpritePos(const DatSpritesGroup* spriteGroup, const uint8_t* ptr, int x, int y, uint32_t num)
{
    ...
    for (uint32_t i = 0; i < spriteGroup->count; ++i)
    {
        ...
        if (num == i)
        {
            ...
            {
                _video->decodeSPR(
                    ptr + 8, _video->_frontLayer, x, y, 0, READ_LE_UINT16(ptr + 4), READ_LE_UINT16(ptr + 6));
            }
            break;
        }
        ptr += size + 2;
...
void Menu::drawSpriteNextFrame(DatSpritesGroup* spriteGroup, int num, int x, int y)
{
    ...
    {
        _video->decodeSPR(
            ptr + 8, _video->_frontLayer, ptr[0] + x, ptr[1] + y, 0, READ_LE_UINT16(ptr + 4), READ_LE_UINT16(ptr + 6));
    }
    ++spriteGroup[num].num;
...
void Menu::drawBitmap(
    const DatBitmapsGroup* bitmapsGroup, const uint8_t* bitmapData, int x, int y, int w, int h, uint8_t baseColor)
{
    if (_res->_isPsx)
    {
...
void Menu::drawBitmapsCircularList(
    const DatBitmapsGroup* bitmapsGroup, const uint8_t* bitmapData, int num, int count, bool updatePalette)
{
...
void Menu::drawCheckpointScreen()
{
    ...
    drawBitmapsCircularList(
        _checkpointsBitmaps[_levelNum],
        _checkpointsBitmapsData[_levelNum],
        _checkpointNum,
        _lastLevelCheckpointNum[_levelNum],
        false);
...
void Menu::handleOptions()
{
    ...
    while (!g_system->inp.quit)
    {
        ...
        if (_optionNum == kMenu_Quit + 1 || _optionNum == kMenu_NewGame + 1 || _optionNum == kMenu_CurrentGame + 1
            || _optionNum == kMenu_ResumeGame)
        {
