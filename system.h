...
struct PlayerInput
{
    ...
    bool keyPressed(int keyMask) const { return (prevMask & keyMask) == 0 && (mask & keyMask) == keyMask; }
    bool keyReleased(int keyMask) const { return (prevMask & keyMask) == keyMask && (mask & keyMask) == 0; }
    void flush() { prevMask = mask = 0; }
};
...
struct System
{
    ...
    virtual void copyYuv(
        int w, int h, const uint8_t* y, int ypitch, const uint8_t* u, int upitch, const uint8_t* v, int vpitch) = 0;
