...
static const char* _prefixes[] = { "rock", "fort", "pwr1", "isld", "lava", "pwr2", "lar1", "lar2", "dark", "test" };
...
void Resource::loadSetupDat()
{
    ...
    for (int i = 0; i < kLvl_dark; ++i)
    { // last level has a single checkpoint
    ...
    if (_loadingImageBuffer)
    {
        ...
        {
            // loading image
            uint32_t size = READ_LE_UINT32(_loadingImageBuffer + offset);
            offset += 8;
            offset += size + 768;

            // loading animation
            size = READ_LE_UINT32(_loadingImageBuffer + offset + 8);
            offset += 16;
            offset += size;
        }
...
void Resource::loadLvlScreenObjectData(LvlObject* dat, const uint8_t* src)
{
    ...
    dat->xPos = READ_LE_UINT32(src);
    src += 4;
    dat->yPos = READ_LE_UINT32(src);
    src += 4;
    dat->screenNum = *src++;
    dat->screenState = *src++;
    dat->dataNum = *src++;
    dat->frame = *src++;
    dat->anim = READ_LE_UINT16(src);
    src += 2;
    dat->type = *src++;
    dat->spriteNum = *src++;
    dat->flags0 = READ_LE_UINT16(src);
    src += 2;
    dat->flags1 = READ_LE_UINT16(src);
    src += 2;
    dat->flags2 = READ_LE_UINT16(src);
    src += 2;
    dat->objectUpdateType = *src++;
    dat->hitCount = *src++;
    const uint32_t objRef = READ_LE_UINT32(src);
    src += 4;
    if (objRef)
    {
    ...
    dat->width = READ_LE_UINT16(src);
    src += 2;
    dat->height = READ_LE_UINT16(src);
    src += 2;
    dat->directionKeyMask = *src++;
    dat->actionKeyMask = *src++;
    dat->currentSprite = READ_LE_UINT16(src);
    src += 2;
    dat->currentSound = READ_LE_UINT16(src);
    src += 2;
    src += 2; // 0x26
    dat->bitmapBits = 0;
    src += 4;
    dat->callbackFuncPtr = 0;
    src += 4;
    dat->dataPtr = 0;
    src += 4;
    dat->sssObject = 0;
    src += 4;
    dat->levelData0x2988 = 0;
    src += 4;
    for (int i = 0; i < 8; ++i)
    {
        dat->posTable[i].x = READ_LE_UINT16(src);
        src += 2;
        dat->posTable[i].y = READ_LE_UINT16(src);
        src += 2;
    }
    dat->nextPtr = 0;
    src += 4;
...
static uint32_t resFixPointersLevelData0x2988(uint8_t* src, uint8_t* ptr, LvlObjectData* dat, bool isPsx)
{
    ...
    dat->unk0 = *src++;
    dat->spriteNum = *src++;
    dat->framesCount = READ_LE_UINT16(src);
    src += 2;
    dat->hotspotsCount = READ_LE_UINT16(src);
    src += 2;
    dat->movesCount = READ_LE_UINT16(src);
    src += 2;
    dat->coordsCount = READ_LE_UINT16(src);
    src += 2;
    dat->refCount = *src++;
    dat->frame = *src++;
    dat->anim = READ_LE_UINT16(src);
    ...
    src += 2; // 0xE
    src += 4; // 0x10
    uint32_t movesDataOffset = READ_LE_UINT32(src);
    src += 4; // 0x14
    src += 4; // 0x18
    uint32_t framesDataOffset = READ_LE_UINT32(src);
    src += 4; // 0x1C
    src += 4; // 0x20
    uint32_t coordsDataOffset = READ_LE_UINT32(src);
    src += 4; // 0x24
    uint32_t hotspotsDataOffset = READ_LE_UINT32(src);
    src += 4; // 0x28
...
void Resource::loadLvlData(File* fp)
{
    ...
    debug(
        kDebug_RESOURCE,
        "Resource::loadLvlData() %d %d %d %d",
        _lvlHdr.screensCount,
        _lvlHdr.staticLvlObjectsCount,
        _lvlHdr.otherLvlObjectsCount,
        _lvlHdr.spritesCount);
...
static uint32_t resFixPointersLevelData0x2B88(
    const uint8_t* src, uint8_t* ptr, uint8_t* offsetsPtr, LvlBackgroundData* dat, bool isPsx)
{
    const uint8_t* start = src;
    ...
    dat->unkB = *src++;
    dat->backgroundPaletteId = READ_LE_UINT16(src);
    src += 2;
    dat->backgroundBitmapId = READ_LE_UINT16(src);
    src += 2;
    for (int i = 0; i < 4; ++i)
    {
        const uint32_t offs = READ_LE_UINT32(src);
        src += 4;
    ...
    for (int i = 0; i < 8; ++i)
    {
        ...
        if (offs != 0)
        {
            ...
            offsetsSize += resFixPointersLevelData0x2988(
                ptr + offs, offsetsPtr + offsetsSize, dat->backgroundLvlObjectDataTable[i], isPsx);
        }
        else
        {
...
void Resource::loadSssData(File* fp, const uint32_t baseOffset)
{
    ...
    debug(
        kDebug_RESOURCE,
        "_sssHdr.bufferSize %d _sssHdr.preloadPcmCount %d _sssHdr.preloadInfoCount %d",
        _sssHdr.bufferSize,
        _sssHdr.preloadPcmCount,
        _sssHdr.preloadInfoCount);
    _sssHdr.infosDataCount = fp->readUint32();
    _sssHdr.filtersDataCount = fp->readUint32();
    _sssHdr.banksDataCount = fp->readUint32();
    debug(
        kDebug_RESOURCE,
        "_sssHdr.infosDataCount %d _sssHdr.filtersDataCount %d _sssHdr.banksDataCount %d",
        _sssHdr.infosDataCount,
        _sssHdr.filtersDataCount,
        _sssHdr.banksDataCount);
    _sssHdr.samplesDataCount = fp->readUint32();
    _sssHdr.codeSize = fp->readUint32();
    debug(
        kDebug_RESOURCE, "_sssHdr.samplesDataCount %d _sssHdr.codeSize %d", _sssHdr.samplesDataCount, _sssHdr.codeSize);
    ...
    for (int i = 0; i < _sssHdr.banksDataCount; ++i)
    {
        ...
        debug(
            kDebug_RESOURCE,
            "SssBank #%d count %d codeOffset 0x%x",
            i,
            _sssBanksData[i].count,
            _sssBanksData[i].firstSampleIndex);
        bytesRead += 8;
    }
    ...
    for (int i = 0; i < _sssHdr.samplesDataCount; ++i)
    {
        ...
        debug(
            kDebug_RESOURCE,
            "SssSample #%d pcm %d frames %d",
            i,
            _sssSamplesData[i].pcm,
            _sssSamplesData[i].framesCount);
        bytesRead += 24;
    }
    ...
    for (int i = 0; i < _sssHdr.preloadInfoCount; ++i)
    {
        ...
        debug(
            kDebug_RESOURCE,
            "_sssPreloadInfosData #%d/%d count %d offset 0x%x",
            i,
            _sssHdr.preloadInfoCount,
            _sssPreloadInfosData[i].count);
        bytesRead += 8;
    }
    if (_sssHdr.version == 10 || _sssHdr.version == 12)
    {
    ...
    for (int i = 0; i < _sssHdr.pcmCount; ++i)
    {
        ...
        debug(
            kDebug_RESOURCE,
            "sssPcmTable #%d/%d offset 0x%x size %d",
            i,
            _sssHdr.pcmCount,
            _sssPcmTable[i].offset,
            _sssPcmTable[i].totalSize);
...
{
    static const uint8_t opcodesLength[] = { 4, 0, 4, 0, 4, 12, 8, 0, 16, 4, 4, 4, 4,  8, 12,
                                             0, 4, 8, 8, 4, 4,  4, 8, 4,  8, 4, 8, 16, 8, 4 };
    int offset = 0;
    while (offset < size)
    {
...
static void decodeSssSpuAdpcmUnit(const uint8_t* src, int16_t* dst)
{ // src: 16bytes, dst: 112bytes
...
void Resource::loadMstData(File* fp)
{
    ...
    for (int i = 0; i < _mstHdr.monsterActionIndexDataCount; ++i)
    {
        ...
        _mstMonsterActionIndexData[i].indexUnk48 =
            (uint32_t*)malloc(_mstMonsterActionIndexData[i].count1 * sizeof(uint32_t));
        fp->skipUint32();
    ...
    for (int i = 0; i < _mstHdr.monsterActionIndexDataCount; ++i)
    {
        ...
        {
            bytesRead +=
                readBytesAlign(fp, _mstMonsterActionIndexData[i].data, _mstMonsterActionIndexData[i].dataCount);
        }
    }
    ...
    for (int i = 0; i < _mstHdr.movingBoundsDataCount; ++i)
    {
        _mstMovingBoundsData[i].data1 =
            (MstMovingBoundsUnk1*)malloc(_mstMovingBoundsData[i].count1 * sizeof(MstMovingBoundsUnk1));
