...
// indexes _mstLut1
static const uint8_t _mstLut3[8 * 5] = { 0x01, 0x03, 0x09, 0x02, 0x08, 0x03, 0x02, 0x01, 0x06, 0x09,
                                         0x02, 0x06, 0x03, 0x04, 0x01, 0x06, 0x04, 0x02, 0x0C, 0x03,
                                         0x04, 0x0C, 0x06, 0x08, 0x02, 0x0C, 0x08, 0x04, 0x09, 0x06,
                                         0x08, 0x09, 0x0C, 0x01, 0x04, 0x09, 0x01, 0x08, 0x03, 0x0C };

// monster frames animation (minus #0, #4, #8, #12, #16, #20, eg. every 4th is skipped)
static const uint8_t _mstLut4[] = { 0x01, 0x02, 0x03, 0x05, 0x06, 0x07, 0x09, 0x0A, 0x0B,
                                    0x0D, 0x0E, 0x0F, 0x11, 0x12, 0x13, 0x15, 0x16, 0x17 };

// indexes _mstLut4 clipped to [0,17], repeat the frame preceding the skipped ones
static const uint8_t _mstLut5[] = { 0x00, 0x00, 0x01, 0x02, 0x03, 0x03, 0x04, 0x05, 0x06, 0x06, 0x07,
                                    0x08, 0x09, 0x09, 0x0A, 0x0B, 0x0C, 0x0C, 0x0D, 0x0E, 0x0F, 0x0F,
                                    0x10, 0x11, 0x12, 0x12, 0x13, 0x14, 0x15, 0x15, 0x16, 0x17 };

static const uint8_t _fireflyPosData[] = { 0, 1, 2, 3,    4, 4, 4, 0xFF, 0, 1, 2, 2,    4, 4, 3, 0xFF, 0, 1, 1, 2,
                                           3, 2, 3, 0xFF, 0, 1, 1, 2,    3, 3, 3, 0xFF, 0, 0, 1, 2,    2, 2, 2, 0xFF,
                                           4, 4, 4, 3,    2, 1, 0, 0xFF, 3, 4, 4, 2,    2, 1, 0, 0xFF, 3, 2, 3, 2,
                                           1, 1, 0, 0xFF, 3, 3, 3, 2,    1, 1, 0, 0xFF, 2, 2, 2, 2,    1, 0, 0, 0xFF };
...
{
    uint8_t r = 0;
    if (a & kDirectionKeyMaskLeft)
    { // Andy left facing monster
        r |= 1;
    }
    if (a & kDirectionKeyMaskDown)
    { // Andy below the monster
        r |= 2;
    }
    return r;
}
...
void Game::mstTaskUpdateScreenPosition(Task* t)
{
    ...
    if (ptr[946] & 4)
    {
        ...
        if (ptr1[0xE] != 0)
        {
            ...
            m->bboxNum[0] = mstBoundingBoxUpdate(
                m->bboxNum[0], m->monster1Index, _mstTemp_x1, _mstTemp_y1, _mstTemp_x2, _mstTemp_y2);
        }
        else
        {
    ...
    if (_andyShootsCount != 0 && !_specialAnimFlag && (o->flags1 & 6) != 6
        && (m->localVars[7] > 0 || m->localVars[7] < -1) && (m->flagsA5 & 0x80) == 0)
    {
        for (int i = 0; i < _andyShootsCount; ++i)
        {
            ...
            if (_al == 1 || _al == 2)
            {
                if (p->monsterDistance >= m->xDelta + m->yDelta)
                {
                    ...
                    {
                        const int dx = _res->_mstPointOffsets[o->screenNum].xOffset
                            - _res->_mstPointOffsets[_currentScreen].xOffset;
                        const int dy = _res->_mstPointOffsets[o->screenNum].yOffset
                            - _res->_mstPointOffsets[_currentScreen].yOffset;
                        _mstTemp_x1 += dx;
                        _mstTemp_x2 += dx;
                        _mstTemp_y1 += dy;
                        _mstTemp_y2 += dy;
                    ...
                    if (rect_intersects(0, 0, 255, 191, _mstTemp_x1, _mstTemp_y1, _mstTemp_x2, _mstTemp_y2))
                    {
                        ...
                        if ((type == 9 && clipLvlObjectsSmall(p->o, o, 132))
                            || (type != 9 && clipLvlObjectsSmall(p->o, o, 20)))
                        {
                            p->m = m;
                    ...
                    if (o->screenNum != _currentScreen)
                    {
                        const int dx = _res->_mstPointOffsets[_currentScreen].xOffset
                            - _res->_mstPointOffsets[o->screenNum].xOffset;
                        const int dy = _res->_mstPointOffsets[_currentScreen].yOffset
                            - _res->_mstPointOffsets[o->screenNum].yOffset;
                        _mstTemp_x1 += dx;
                        _mstTemp_x2 += dx;
                        _mstTemp_y1 += dy;
                        _mstTemp_y2 += dy;
            ...
            else if (_al == 3 && p->monsterDistance > m->xDelta + m->yDelta)
            {
                ...
                {
                    const int dx =
                        _res->_mstPointOffsets[o->screenNum].xOffset - _res->_mstPointOffsets[_currentScreen].xOffset;
                    const int dy =
                        _res->_mstPointOffsets[o->screenNum].yOffset - _res->_mstPointOffsets[_currentScreen].yOffset;
                    _mstTemp_x1 += dx;
                    _mstTemp_x2 += dx;
                    _mstTemp_y1 += dy;
                    _mstTemp_y2 += dy;
                ...
                const int dx =
                    _res->_mstPointOffsets[_currentScreen].xOffset - _res->_mstPointOffsets[o->screenNum].xOffset;
                const int dy =
                    _res->_mstPointOffsets[_currentScreen].yOffset - _res->_mstPointOffsets[o->screenNum].yOffset;
                _mstTemp_x1 += dx;
                _mstTemp_x2 += dx;
                _mstTemp_y1 += dy;
                _mstTemp_y2 += dy;
            ...
            if (m->collideDistance == -1 || m->collideDistance > res
                || (m->collideDistance == 0 && res == 0 && (m->shootData->type & 1) == 0 && p->type == 2))
            {
                m->shootData = p;
    ...
    // m->unk90 =
    // m->unk8C =
    if (_andyShootsTable[0].type == 3 && m->shootSource != 0 && _andyShootsTable[0].directionMask == m->shootDirection
        && m->directionKeyMask == _andyObject->directionKeyMask)
    {
        m->flags48 |= 0x80;
    }
    else
...
int Game::mstWalkPathUpdateWalkNode(MstWalkPath* walkPath, MstWalkNode* walkNode, int num, int index)
{
    if (walkNode->coords[num][index] < 0)
    {
        ...
        else
        {
            ...
            walkNode->coords[num][index] =
                mstWalkPathUpdateWalkNode(walkPath, &walkPath->data[indexWalkNode], num, index);
        }
    }
...
void Game::mstMonster1UpdateGoalPosition(MonsterObject1* m)
{
    ...
    debug(
        kDebug_MONSTER,
        "mstMonster1UpdateGoalPosition m %p goalScreen %d levelBounds [%d,%d,%d,%d]",
        m,
        m->goalScreenNum,
        m->levelPosBounds_x1,
        m->levelPosBounds_y1,
    ...
    if (m->goalScreenNum == 0xFD)
    {
        ...
        if (m->levelPosBounds_x2 > xPos_1 && m->levelPosBounds_x1 < xPos_2 && m->levelPosBounds_y2 > yPos_1
            && m->levelPosBounds_y1 < yPos_2)
        {
    ...
    debug(
        kDebug_MONSTER,
        "mstMonster1UpdateGoalPosition m %p mask 0x%x goal [%d,%d,%d,%d]",
        m,
        var1C,
        m->goalPos_x1,
        m->goalPos_y1,
...
void Game::mstMonster1MoveTowardsGoal1(MonsterObject1* m)
{
    ...
    debug(
        kDebug_MONSTER,
        "mstMonster1MoveTowardsGoal1 m %p pos %d,%d [%d,%d,%d,%d]",
        m,
        m->xMstPos,
        m->yMstPos,
        m34->left,
    ...
    debug(
        kDebug_MONSTER,
        "mstMonster1MoveTowardsGoal1 m %p pos %d,%d dist %d,%d direction 0x%x",
        m,
        _xMstPos1,
        _yMstPos1,
        _xMstPos2,
...
bool Game::mstMonster1TestGoalDirection(MonsterObject1* m)
{
    ...
    else
    {
        ...
        {
            m->m49Unk1 = &m->m49->data1[m->indexUnk49Unk1];
            if (((m->goalDirectionMask & kDirectionKeyMaskHorizontal) == 0 || _xMstPos2 >= m->m49Unk1->unkC)
                && ((m->goalDirectionMask & kDirectionKeyMaskVertical) == 0 || _yMstPos2 >= m->m49Unk1->unkD))
            {
                return true;
            }
        }
    ...
    for (int var20 = 0; var20 < 5; ++var20)
    {
        ...
        if (p[0xE] != 0)
        {
            ...
            if (r != 0)
            {
                ...
                if (rect_intersects(
                        b->x1, b->y1, b->x2, b->y2, m->goalPos_x1, m->goalPos_y1, m->goalPos_x2, m->goalPos_y2))
                {
                    break;
                }
                continue;
...
int Game::mstTaskStopMonster1(Task* t, MonsterObject1* m)
{
    ...
    if (m->goalScreenNum != 0xFC && (m->flagsA5 & 8) != 0 && (t->flags & 0x20) != 0 && m->action)
    {
        ...
        if (xPosObj < xPosScreen || xPosObj + o->width - 1 > xPosScreen + 255 || yPosObj < yPosScreen
            || yPosObj + o->height - 1 > yPosScreen + 191)
        {
...
bool Game::lvlObjectCollidesAndy1(LvlObject* o, int flags) const
{
    ...
    else
    {
        ...
        {
            const int dx =
                _res->_mstPointOffsets[o->screenNum].xOffset - _res->_mstPointOffsets[_andyObject->screenNum].xOffset;
            x1 += dx;
            x2 += dx;
            const int dy =
                _res->_mstPointOffsets[o->screenNum].yOffset - _res->_mstPointOffsets[_andyObject->screenNum].yOffset;
            y1 += dy;
            y2 += dy;
        }
    }
...
bool Game::lvlObjectCollidesAndy2(LvlObject* o, int type) const
{
    ...
    else
    {
        ...
        {
            const int dx =
                _res->_mstPointOffsets[_andyObject->screenNum].xOffset - _res->_mstPointOffsets[o->screenNum].xOffset;
            x1 += dx;
            x2 += dx;
            const int dy =
                _res->_mstPointOffsets[_andyObject->screenNum].yOffset - _res->_mstPointOffsets[o->screenNum].yOffset;
            y1 += dy;
            y2 += dy;
        }
    }
    return rect_intersects(
        ...
        _andyObject->xPos,
        _andyObject->yPos,
        _andyObject->xPos + _andyObject->width - 1,
        _andyObject->yPos + _andyObject->height - 1);
}
...
bool Game::mstCollidesByFlags(MonsterObject1* m, uint32_t flags)
{
    ...
    else if ((flags & 0x200) != 0 && (_mstFlags & 0x40000000) != 0)
    {
        ...
        (flags & 0x40) != 0 && (mstGetFacingDirectionMask(m->facingDirectionMask) & 1) != ((m->o16->flags1 >> 4) & 1)
        && (m->monsterInfos[946] & 4) == 0)
    {
        return false;
    }
    else if (
        (flags & 0x80) != 0 && (mstGetFacingDirectionMask(m->facingDirectionMask) & 1) != ((m->o16->flags1 >> 4) & 1)
        && (m->monsterInfos[946] & 4) != 0)
    {
        return false;
    }
    else if (
        (flags & 0x400) != 0 && (m->o16->screenNum != _andyObject->screenNum || !lvlObjectCollidesAndy1(m->o16, 0)))
    {
        return false;
    }
    else if (
        (flags & 0x800) != 0 && (m->o16->screenNum != _andyObject->screenNum || !lvlObjectCollidesAndy1(m->o16, 1)))
    {
        return false;
    }
    else if (
        (flags & 0x100000) != 0 && (m->o16->screenNum != _andyObject->screenNum || !lvlObjectCollidesAndy3(m->o16, 0)))
    {
        return false;
    }
    else if (
        (flags & 0x200000) != 0 && (m->o16->screenNum != _andyObject->screenNum || !lvlObjectCollidesAndy3(m->o16, 1)))
    {
        return false;
    }
    ...
    else if ((flags & 0x20) != 0 && (m->o16->flags0 & 0x100) == 0)
    {
        ...
        (flags & 0x10000) != 0 && (m->o16->screenNum != _andyObject->screenNum || !lvlObjectCollidesAndy4(m->o16, 1)))
    {
        return false;
    }
    else if (
        (flags & 0x20000) != 0 && (m->o16->screenNum != _andyObject->screenNum || !lvlObjectCollidesAndy4(m->o16, 0)))
    {
        return false;
    }
    else if (
        (flags & 0x40000) != 0
        && (m->o16->screenNum != _andyObject->screenNum || !clipLvlObjectsBoundingBox(_andyObject, m->o16, 36)))
    {
        return false;
    }
    else if (
        (flags & 0x80000) != 0
        && (m->o16->screenNum != _andyObject->screenNum || !clipLvlObjectsBoundingBox(_andyObject, m->o16, 20)))
    {
        return false;
    }
    return true;
...
int Game::mstUpdateTaskMonsterObject1(Task* t)
{
    ...
    for (int i = 0; i < _andyShootsCount; ++i)
    {
        ...
        if (t->run != &Game::mstTask_monsterWait5 && t->run != &Game::mstTask_monsterWait6
            && t->run != &Game::mstTask_monsterWait7 && t->run != &Game::mstTask_monsterWait8
            && t->run != &Game::mstTask_monsterWait9 && t->run != &Game::mstTask_monsterWait10)
        {
            if (m->monsterInfos[946] & 2)
            {
        ...
        int var20 = -1;
        int indexUnk51;
        if ((m->flagsA5 & 8) != 0 && m->action && m->action->indexUnk51 != kNone
            && m->monsterInfos == &_res->_mstMonsterInfos[m->action->unk0 * 948])
        {
        ...
        for (; var24 < var18->count; ++var24)
        {
            ...
            if (hSize != 0 || vSize != 0)
            {
                int dirMask = 0;
                int ve = 0;
                if ((m50Unk1->unk4 & kDirectionKeyMaskHorizontal) != kDirectionKeyMaskHorizontal
                    && (m->o16->flags1 & 0x10) != 0)
                {
            ...
            if ((m->monsterInfos[946] & 4) != 0
                && mstBoundingBoxCollides1(m->monster1Index, _mstTemp_x1, _mstTemp_y1, _mstTemp_x2, _mstTemp_y2) != 0)
            {
                continue;
            }
            if (m50Unk1->width != 0
                && getMstDistance((m->monsterInfos[946] & 2) != 0 ? var14->boundingBox.y2 : m->yMstPos, var14) >= 0)
            {
                continue;
            }
    ...
    if (dir == 1)
    {
        ...
        else if (ve == 1)
        {
            ...
            {
                if ((_mstCurrentPosX > m->xMstPos && _mstCurrentPosX > m->walkNode->coords[0][1])
                    || (_mstCurrentPosX < m->xMstPos && _mstCurrentPosX < m->walkNode->coords[1][1]))
                {
    ...
    if ((m->flagsA5 & 8) == 0 && (m->monsterInfos[946] & 2) == 0)
    {
        ...
        {
            if ((int32_t)READ_LE_UINT32(m->monsterInfos + 916) <= m->walkNode->coords[1][1]
                || (int32_t)READ_LE_UINT32(m->monsterInfos + 912) >= m->walkNode->coords[0][1])
            {
                m->flagsA6 |= 1;
        ...
        {
            if ((int32_t)READ_LE_UINT32(m->monsterInfos + 920) >= m->walkNode->coords[0][1]
                || (int32_t)READ_LE_UINT32(m->monsterInfos + 924) <= m->walkNode->coords[1][1])
            {
                m->flagsA6 |= 1;
                assert(m == _mstCurrentMonster1);
                m->flagsA5 = 1;
...
void Game::mstUpdateRefPos()
{
    if (_andyObject)
    {
        ...
        if (!_specialAnimFlag)
        {
            _mstAndyRectNum = mstBoundingBoxUpdate(
                                  _mstAndyRectNum,
                                  0xFE,
                                  _mstAndyLevelPosX,
                                  _mstAndyLevelPosY,
                                  _mstAndyLevelPosX + _andyObject->width - 1,
                                  _mstAndyLevelPosY + _andyObject->height - 1)
                & 0xFF;
        }
    ...
    if (!_lvlObjectsList0)
    {
        ...
        _andyShootsTable[0].xPos =
            _plasmaCannonPosX[_plasmaCannonFirstIndex] + _res->_mstPointOffsets[_currentScreen].xOffset;
        _andyShootsTable[0].yPos =
            _plasmaCannonPosY[_plasmaCannonFirstIndex] + _res->_mstPointOffsets[_currentScreen].yOffset;
    ...
    if (m->flagsA5 & 8)
    {
        ...
        if (n)
        {
            ...
            if (t->run != &Game::mstTask_wait2)
            {
                ...
                if ((t->flags & 0x40) != 0 || p[0] == 203
                    || ((flags & 0x10) != 0
                        && (t->run == &Game::mstTask_monsterWait1 || t->run == &Game::mstTask_monsterWait2
                            || t->run == &Game::mstTask_monsterWait3 || t->run == &Game::mstTask_monsterWait4)))
                {
                    p += 4;
                }
                n->codeData = p;
...
int Game::mstTaskSetActionDirection(Task* t, int num, int delay)
{
    ...
    debug(
        kDebug_MONSTER,
        "mstTaskSetActionDirection m %p action 0x%x direction 0x%x (%d,%d)",
        m,
        o->actionKeyMask,
        o->directionKeyMask,
        vf,
    ...
    if (vf != 0 || ve != 0)
    {
        int dirMask = 0;
        uint8_t var11 = p[2];
        if (((var11 & kDirectionKeyMaskHorizontal) == kDirectionKeyMaskHorizontal && (o->directionKeyMask & 8) != 0)
            || ((var11 & kDirectionKeyMaskHorizontal) != kDirectionKeyMaskHorizontal && (o->flags1 & 0x10) != 0))
        {
            vf = m->xMstPos - vf;
        }
        else
...
int Game::mstTask_main(Task* t)
{
    ...
    do
    {
        ...
        case 40: // 27 - remove_monsters_screen_type
            ...
            break;
        case 41:
        { // 28 - increment_task_var
            assert(p[1] < kMaxLocals);
            ++t->localVars[p[1]];
        }
        break;
        case 42:
        { // 29 - increment_global_var
            const int num = p[1];
            assert(num < kMaxVars);
            ++_mstVars[num];
        }
        ...
        { // 59
            ...
            int dirMask = 0;
            if ((t->monster1 && (t->monster1->monsterInfos[944] == 10 || t->monster1->monsterInfos[944] == 25))
                || (t->monster2 && (t->monster2->monster2Info->type == 10 || t->monster2->monster2Info->type == 25)))
            {
        ...
        break;
        case 213:
        { // 60 - monster_set_action_direction
            LvlObject* o = 0;
            if (t->monster2)
            {
                o = t->monster2->o;
        ...
        { // 68 - add_monster_group
            ...
            if (countType1 != 0 || countType2 != 0)
            {
                mstOp68_addMonsterGroup(
                    t,
                    _res->_mstMonsterInfos + m226Data->unk0 * kMonsterInfoDataSize,
                    countType1,
                    countType2,
                    vc,
                    m226Data->unk6);
...
int Game::mstOp49_setMovingBounds(int a, int b, int c, int d, int screen, Task* t, int num)
{
    ...
    {
        if (t->run != &Game::mstTask_monsterWait6 && t->run != &Game::mstTask_monsterWait8
            && t->run != &Game::mstTask_monsterWait10)
        {
    ...
    for (int i = 0; i < m48->areaCount; ++i)
    {
        ...
        if (num != 0xFF)
        {
            ...
            if ((m->flagsA5 & 0x70) == 0)
            {
                ...
                {
                    if (t->run != &Game::mstTask_monsterWait1 && t->run != &Game::mstTask_monsterWait4
                        && t->run != &Game::mstTask_monsterWait2 && t->run != &Game::mstTask_monsterWait3
                        && t->run != &Game::mstTask_monsterWait5 && t->run != &Game::mstTask_monsterWait6
                        && t->run != &Game::mstTask_monsterWait7 && t->run != &Game::mstTask_monsterWait8
                        && t->run != &Game::mstTask_monsterWait9 && t->run != &Game::mstTask_monsterWait10)
                    {
                        m->flagsA5 = (m->flagsA5 & ~0xF) | 6;
...
bool Game::mstHasMonsterInRange(const MstMonsterAction* m48, uint8_t flag)
{
    ...
    for (int i = 0; i < m48->areaCount; ++i)
    {
        ...
        l1:
            ...
            debug(
                kDebug_MONSTER,
                "mstHasMonsterInRange (unk0!=0) count:%d %d %d [%d,%d] screen:%d",
                m12->count,
                vb,
                vg,
                _mstPosXmin,
            ...
            if (vb >= _mstPosXmin && vb <= _mstPosXmax)
            {
                ...
                if (var4D == 0 || (vg >= _mstPosYmin && vg <= _mstPosYmax))
                {
                    ...
                    for (int j = 0; j < var10; ++j)
                    {
                        ...
                        if (_op54Data[m->monster1Index] == 0
                            && (m12u4->screenNum < 0 || m->o16->screenNum == m12u4->screenNum))
                        {
                            ...
                            if ((var8 || var4) && m->monsterInfos[944] != 10 && m->monsterInfos[944] != 16
                                && m->monsterInfos[944] != 9)
                            {
                                if (vg <= 0)
                                {
...
int Game::mstOp56_specialAction(Task* t, int code, int num)
{
    ...
    case 0:
        if (!_specialAnimFlag && setAndySpecialAnimation(0x71) != 0)
        {
            ...
            {
                _specialAnimMask = merge_bits(
                    _specialAnimMask,
                    _andyObject->flags1,
                    0x30); // _specialAnimMask ^= (_specialAnimMask ^ _andyObject->flags1) & 0x30;
...
void Game::mstOp59_addShootFireball(int x, int y, int screenNum, int type, int state, uint16_t flags)
{
    ...
    if (o)
    {
        ...
        assert(s);
        s->state = state;
        static const uint8_t fireballDxDy1[16] = { 0x0A, 0x00, 0xF7, 0xFA, 0xF7, 0x06, 0x09, 0xFA,
                                                   0x09, 0x06, 0xF6, 0x00, 0x00, 0xF6, 0x00, 0x0A };
        static const uint8_t fireballDxDy2[16] = { 0x0D, 0x00, 0xF5, 0xF9, 0xF5, 0x07, 0x0B, 0xF9,
                                                   0x0B, 0x07, 0xF3, 0x00, 0x00, 0xF3, 0x00, 0x0D };
        static const uint8_t data1[16] = { 0x02, 0x00, 0x01, 0x01, 0x01, 0x03, 0x01, 0x00,
                                           0x01, 0x02, 0x02, 0x01, 0x00, 0x00, 0x00, 0x02 };
        static const uint8_t data2[16] = { 0x0D, 0x00, 0x0D, 0x01, 0x0D, 0x03, 0x0D, 0x00,
                                           0x0D, 0x02, 0x0D, 0x01, 0x0D, 0x00, 0x0D, 0x02 };
        assert(state < 8);
        const uint8_t* anim;
        if (type >= 7)
        {
...
void Game::mstResetCollisionTable()
{
    ...
    for (int i = 0; i < kMaxMonsterObjects1; ++i)
    {
        ...
        if ((_bl & 2) != 0
            || ((_bl & 1) != 0
                && ((m->walkNode->walkCodeStage1 == kNone && !m->walkCode)
                    || (m->walkNode->walkCodeStage1 != kNone
                        && m->walkCode == &_res->_mstWalkCodeData[m->walkNode->walkCodeStage1]))))
        {
            if ((_bl & 0xB0) != 0)
            {
                continue;
    ...
    int arg24)
{
    debug(
        kDebug_MONSTER,
        "mstOp67_addMonster pos %d,%d,%d,%d %d %d 0x%x 0x%x %d %d %d",
        y1,
        x1,
        y2,
        x2,
    ...
    if (arg1C != -128)
    {
        ...
        { // Master of Darkness
            m->o20 =
                addLvlObject(ptr[945], x1, y1, objScreen, ptr[944], behaviorState->anim + 1, o_flags1, 0x3001, 0, 0);
            if (!m->o20)
            {
